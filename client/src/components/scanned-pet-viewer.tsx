...
export default function ScannedPetViewer({
  data,
  onClose,
  onDelete,
  onTransfer,
}: ScannedPetViewerProps) {
  ...
  const { data: medicalRecords, isLoading: loadingRecords } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/pets/public/${data.petId}/medical-records`,
      );
      if (!response.ok) {
        ...
        <CardContent className="space-y-4">
          <div className="text-center">
            ...
            <div className="mt-2 text-xs text-gray-500">
              <div>
                Scanned:{" "}
                {format(new Date(data.scannedAt), "MMM d, yyyy h:mm a")}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {currentData.owner.phone && (
                      <div className="flex justify-between items-center">
                        ...
                        <a
                          href={`tel:${currentData.owner.phone}`}
                    ...
                    {currentData.owner.email && (
                      <div className="flex justify-between items-center">
                        ...
                        <a
                          href={`mailto:${currentData.owner.email}`}
                          className="font-medium text-blue-600 hover:underline flex items-center"
                        >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {currentData.owner.emergencyPhone && (
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600">Phone:</span>
                      <a
                        href={`tel:${currentData.owner.emergencyPhone}`}
            ...
            <div className="border-t pt-3">
              ...
              <div className="text-sm text-gray-600">
                Scanned on{" "}
                {data.scannedAt
                  ? format(new Date(data.scannedAt), "MMM dd, yyyy 'at' h:mm a")
                  : "Unknown date"}
              </div>
            </div>
          </div>
